...
- **`make test` runs the harness in `test/`** with `--host
  wippy.terminal:host`; it writes `test/shots/run.png`, the dialog as the
  shell's renderer drew it, and boots a live compositor for the Start menu
  and a real Bash under a PTY. Look at the picture: the geometry checks do
  not see a wrong colour or a caption a pixel off.
