-- The real compositor of the desktop base with the shell's theme on the
-- test viewport. It wraps the theme's `paint` to report to the test what a
-- person would see: the Start menu's hit spots by entry (so the test clicks
-- a menu row by its id, not by a guessed coordinate) and the Run dialog's
-- frame — its state tree and its client rectangle — whenever it is drawn.
...
local function main(service, observer, mode)
    ...
    chrome.paint = function(state, cw, ch)
        ...
        for _, window in ipairs(state.windows) do
            if window.entry == RUN and window.content_state and not window.waiting then
