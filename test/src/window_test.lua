...
local function define_tests()
    test.describe("Run window", function()
        test.it("is a dialog at the root of Start with its own picture at both sizes", function()
            local entry = assert(registry.get("chicago.run:window"))
            ...
            test.eq(meta.image, "chicago.run:images/run", "the picture is the module's own pack")
            test.eq(meta.pixel_render .. "|" .. meta.pixel_state, "chicago.shell.sdk:render|chicago.run:window")
            ...
            local missing, reason = images.get("chicago.run:images/nothing", 16)
            test.is_nil(missing)
