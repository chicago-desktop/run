-- The Run dialog: the launch spec, the SDK layout and the default-button
-- rule, "Browse…" against a substituted compositor, and then the real thing —
-- the Start menu of a live compositor, a Bash window under a PTY launched
-- from the dialog and still alive after the dialog closed, Esc in cells.
--
...
-- green in under a millisecond, and never executed.
...
-- Polls `check` until it answers something truthy and returns that answer:
-- a window row, a screen, or just `true`, so the type is open on purpose.
local function eventually(check: any): any
    local deadline = time.now():unix_nano() + 8000000000
    while time.now():unix_nano() < deadline do
        local value: any = check()
