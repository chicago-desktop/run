...
- **The launch spec is data, the dialog is thin.** What a test should see
  without a compositor goes into the pure `model` library (`src/model.lua`)
  and the `definition` table `src/window.lua` exports next to `app.main`;
  the request to the compositor goes through `definition.request`, which a
  test substitutes. Use the SDK as documented in `docs/sdk.md` and the
